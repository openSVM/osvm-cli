...
fn test_svm_deploy_workflow_structure() {
    ...
    assert!(Path::new(workflow_path).exists(), "SVM deploy workflow file should exist");
    ...
    assert!(workflow.get("name").is_some(), "Workflow should have a name");
    assert!(workflow.get("on").is_some(), "Workflow should have triggers");
    ...
    assert!(on_section.get("workflow_call").is_some(), "Should be a reusable workflow");
}
...
fn test_workflow_inputs() {
    ...
    for input in required_inputs {
        assert!(inputs.get(input).is_some(), "Required input '{}' should be defined", input);
        
        let input_def = inputs.get(input).unwrap();
        assert!(input_def.get("required").is_some(), "Input '{}' should have required field", input);
        assert!(input_def.get("type").is_some(), "Input '{}' should have type field", input);
    }
    ...
    for input in optional_inputs {
        if let Some(input_def) = inputs.get(input) {
            assert!(input_def.get("default").is_some(), "Optional input '{}' should have default", input);
        }
    }
}
...
fn test_workflow_secrets() {
    ...
    assert!(secrets.get("SSH_PRIVATE_KEY").is_some(), "SSH_PRIVATE_KEY secret should be defined");
    
    let ssh_key_def = secrets.get("SSH_PRIVATE_KEY").unwrap();
    assert!(ssh_key_def.get("required").is_some(), "SSH_PRIVATE_KEY should have required field");
    
    // Check that SSH_PRIVATE_KEY is required
    let required = ssh_key_def.get("required").unwrap().as_bool().unwrap_or(false);
...
fn test_ci_workflow_structure() {
    ...
    assert!(Path::new(workflow_path).exists(), "CI workflow file should exist");
    
    let content = fs::read_to_string(workflow_path).unwrap();
    let workflow: serde_yaml::Value = serde_yaml::from_str(&content).unwrap();
    
    // Validate CI workflow structure
    assert!(workflow.get("name").is_some(), "CI workflow should have a name");
    
    let on_section = workflow.get("on").unwrap();
    assert!(on_section.get("push").is_some(), "Should trigger on push");
    assert!(on_section.get("pull_request").is_some(), "Should trigger on pull requests");
...
fn test_ci_includes_tests() {
    ...
    let job_names: Vec<&str> = jobs.as_mapping().unwrap().keys()
        .filter_map(|k| k.as_str())
        .collect();
    
    // Should have some form of testing
    let has_test_job = job_names.iter().any(|&name| 
        name.contains("test") || 
        name.contains("check") || 
        name.contains("sanity")
    );
    
    assert!(has_test_job, "CI should include testing jobs. Found jobs: {:?}", job_names);
}
...
fn test_workflow_environment_variables() {
    ...
    if let Some(env) = workflow.get("env") {
        ...
        assert!(env.get("CARGO_TERM_COLOR").is_some(), "Should set cargo color output");
        assert!(env.get("RUST_BACKTRACE").is_some(), "Should enable rust backtraces");
    }
}
...
fn test_workflow_job_strategy() {
    ...
    for (job_name, job_def) in jobs.as_mapping().unwrap() {
        ...
        assert!(job_def.get("runs-on").is_some(), "Job '{}' should specify runs-on", job_name);
        
        // Each job should have steps
        assert!(job_def.get("steps").is_some(), "Job '{}' should have steps", job_name);
        
        let steps = job_def.get("steps").unwrap().as_sequence().unwrap();
        assert!(!steps.is_empty(), "Job '{}' should have at least one step", job_name);
    }
}
...
fn test_cross_platform_workflow_exists() {
    ...
    if Path::new(workflow_path).exists() {
        ...
        assert!(workflow.get("name").is_some(), "Cross-platform workflow should have a name");
                ...
                if let Some(uses) = step.get("uses") {
                    ...
                    if action.starts_with("actions/") {
                        ...
                        assert!(action.contains("@"), 
                            ...
                            action, job_name, step_idx);
                    } else if action.starts_with("./") {
                        // Local actions should exist (we can't verify this in tests but format is correct)
                        assert!(action.starts_with("./.github/"), 
                            "Local action '{}' should be in .github/ directory", action);
                    }
                }
            }
        }
    ...
    if let Some(workflow_call) = workflow.get("on").and_then(|on| on.get("workflow_call")) {
        if let Some(outputs) = workflow_call.get("outputs") {
            ...
            for (output_name, output_def) in outputs.as_mapping().unwrap() {
                ...
                assert!(output_def.get("description").is_some(), 
                    "Output '{}' should have description", output_name);
                assert!(output_def.get("value").is_some(), 
                    "Output '{}' should have value reference", output_name);
            }
        }
    }
}
        ...
        for step in steps {
            ...
            if let Some(run_cmd) = step.get("run") {
                ...
                assert!(!cmd.contains("-----BEGIN"), 
                    "Job '{}' should not contain raw private keys in run commands", job_name);
                assert!(!cmd.contains("sk_"), 
                    "Job '{}' should not contain raw API keys in run commands", job_name);
            }
                ...
                for (env_name, env_value) in env.as_mapping().unwrap() {
                    ...
                    if env_name.as_str().unwrap().to_lowercase().contains("key") ||
                       env_name.as_str().unwrap().to_lowercase().contains("secret") {
                        assert!(env_value_str.contains("secrets.") || env_value_str.contains("inputs."),
    ...
    for workflow_path in &workflow_paths {
        ...
        if let Some(permissions) = workflow.get("permissions") {
            ...
            if let Some(contents) = permissions.get("contents") {
                ...
                assert!(["read", "none"].contains(&contents_perm), 
