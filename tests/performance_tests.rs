...
fn test_performance_thresholds() {
    ...
    assert!(load_duration < Duration::from_millis(100), 
        "Program loading took too long: {:?}", load_duration);
    ...
    assert!(id_load_duration < Duration::from_millis(10),
        "Program ID loading took too long: {:?}", id_load_duration);
}
...
fn test_rpc_cache_performance() {
    ...
    assert!(cache_hit_duration < cache_miss_duration / 10,
    ...
    assert!(cache_hit_duration < Duration::from_millis(1),
        "Cache hit too slow: {:?}", cache_hit_duration);
}
...
fn test_json_performance() {
    ...
    assert!(serialize_duration < Duration::from_millis(1),
        "JSON serialization too slow: {:?}", serialize_duration);
    ...
    assert!(deserialize_duration < Duration::from_millis(1),
        "JSON deserialization too slow: {:?}", deserialize_duration);
}
...
fn test_config_creation_performance() {
    ...
    assert!(total_duration < Duration::from_millis(10),
        "Config creation too slow: {:?} per config", per_config_duration);
}
...
fn test_large_file_performance_degradation() {
    ...
    assert!(ratio <= 2.1, 
        ...
        ratio, baseline_duration, large_duration);
}
...
fn test_serialization_performance_at_scale() {
    ...
    assert!(serialize_duration < Duration::from_millis(10),
        "Batch serialization too slow: {:?}", serialize_duration);
    ...
    assert!(deserialize_duration < Duration::from_millis(10),
        "Batch deserialization too slow: {:?}", deserialize_duration);
}
...
fn test_config_validation_performance() {
    ...
    for i in 0..1000 {
        let config = DeployConfig {
            ...
            publish_idl: i % 2 == 0,
            idl_file_path: if i % 3 == 0 { Some(format!("idl_{}.json", i)) } else { None },
            network_selection: if i % 4 == 0 { "all".to_string() } else { "devnet".to_string() },
            json_output: i % 5 == 0,
    ...
    assert!(validation_duration < Duration::from_millis(5),
        "Config validation too slow: {:?}", validation_duration);
}