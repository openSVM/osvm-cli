...
async fn test_single_query_plan_generation() {
    ...
    println!(
        "\n📊 Messages in session after query: {}",
        messages_after.len()
    );
    println!(
        "📊 NEW messages from this query: {}",
    ...
    println!(
        "   ✓ Found our user message: {}",
        if found_our_user_message { "✅" } else { "❌" }
    );
    ...
    assert!(
        final_responses > 0,
        "Should have at least one final response"
    );

    // Check if plan was generated (it's okay if not - depends on AI decision)
    if !plans.is_empty() {
        println!(
            "\n✅ SUCCESS: Plan WAS generated with {} tools",
            plans.len()
        );
        assert!(
