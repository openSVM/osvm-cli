...
mod tests {
    ...
    fn test_invalid_ovsm_syntax() {
        ...
        for invalid_code in invalid_cases {
            ...
            assert!(
                result.is_err(),
                "Invalid OVSM code '{}' should fail validation",
                invalid_code
            );
        }
    }
...
```
    ...
    fn test_semantic_result_validation() {
        ...
        let (valid, msg) =
            AiService::validate_ovsm_result("null", "Get account balance", "What is my balance?");
        ...
        // Test valid result
        let (valid, _) =
            AiService::validate_ovsm_result("100.5", "Get balance", "What is the balance?");
...
<ovsm_plan>
  ...
  <code>
    ...
    stats
        ...
        assert!(
            result.is_ok(),
            "Syntactically valid plan should pass validation"
        );
...
<ovsm_plan>
  ...
  <code>
    ...
    (+ x 5)
        ...
        assert!(
            result.is_err(),
            "Plan with syntax error should fail validation"
        );
    }
