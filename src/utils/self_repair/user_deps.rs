...
impl UserDependencyManager {
    ...
    pub async fn check_all_user_dependencies(&self) -> Result<SolanaConfigInfo, UserDepsError> {
        ...
        let keypair_path = if config_file_exists {
            self.get_keypair_path_from_config(&config_file).await
    ...
    pub async fn create_solana_config_dir(&self) -> Result<String, UserDepsError> {
        ...
        fs::create_dir_all(&config_dir)
            .map_err(|e| UserDepsError::FileSystemError(format!(
        ...
        Ok(format!("Created Solana config directory: {}", config_dir.display()))
    }

    /// Generate a new Solana keypair
    pub async fn generate_keypair(&self, output_path: Option<&str>) -> Result<String, UserDepsError> {
        ...
        let output = Command::new("solana-keygen")
            ...
            .arg("--no-passphrase")
            .output()
            .map_err(|e| UserDepsError::CommandFailed(format!(
                "Failed to execute solana-keygen: {}", e
            )))?;

        if output.status.success() {
            let pubkey = self.get_pubkey_from_keypair(&keypair_path).await
            ...
            Err(UserDepsError::KeypairError(format!(
                "Failed to generate keypair: {}", error_msg
            )))
        }
    }
    ...
    async fn get_pubkey_from_keypair(&self, keypair_path: &Path) -> Result<String, UserDepsError> {
        let output = Command::new("solana-keygen")
            ...
            .output()
            .map_err(|e| UserDepsError::CommandFailed(format!(
                "Failed to get pubkey from keypair: {}", e
            )))?;
    ...
    pub async fn configure_solana_network(&self, network: &str) -> Result<String, UserDepsError> {
        let rpc_url = match network.to_lowercase().as_str() {
            ...
            _ => return Err(UserDepsError::ConfigError(format!("Unknown network: {}", network))),
        };
        ...
        let output = Command::new("solana")
            ...
            .output()
            .map_err(|e| UserDepsError::CommandFailed(format!(
                "Failed to set Solana network: {}", e
            )))?;
        ...
        if output.status.success() {
            ...
            Err(UserDepsError::ConfigError(format!(
                "Failed to set network: {}", error_msg
            )))
        }
    }
    ...
    async fn get_keypair_path_from_config(&self, config_file: &Path) -> Result<String, UserDepsError> {
        let config_content = fs::read_to_string(config_file)
            .map_err(|e| UserDepsError::ConfigError(format!(
        ...
        if let Ok(config) = serde_yaml::from_str::<serde_yaml::Value>(&config_content) {
            if let Some(keypair_path) = config.get("keypair_path")
                .and_then(|v| v.as_str()) {
        ...
        Ok(self.get_solana_config_dir().join("id.json").to_string_lossy().to_string())
    }
    ...
    pub async fn set_keypair_path(&self, keypair_path: &str) -> Result<String, UserDepsError> {
        ...
        if output.status.success() {
            ...
            let error_msg = String::from_utf8_lossy(&output.stderr);
            Err(UserDepsError::ConfigError(format!(
                "Failed to set keypair path: {}", error_msg
            )))
        }
    }
    ...
    pub async fn install_solana_cli(&self) -> Result<String, UserDepsError> {
        ...
        let output = Command::new("sh")
            ...
            .output()
            .map_err(|e| UserDepsError::CommandFailed(format!(
                "Failed to download Solana installer: {}", e
            )))?;

        if !output.status.success() {
            let error_msg = String::from_utf8_lossy(&output.stderr);
            return Err(UserDepsError::CommandFailed(format!(
                "Solana installation failed: {}", error_msg
            )));
        }
        ...
        if self.is_solana_cli_installed().await? {
            ...
            Err(UserDepsError::CommandFailed(
                "Solana CLI installation verification failed. Please restart your terminal.".to_string()
            ))
        }
    }

    /// Create a complete Solana setup (config directory + keypair + network config)
    pub async fn setup_solana_complete(&self, network: Option<&str>) -> Result<Vec<String>, UserDepsError> {
...
mod tests {
    ...
    async fn test_solana_cli_check() {
        let manager = UserDependencyManager::new();
        let is_installed = manager.is_solana_cli_installed().await
    ...
    async fn test_config_validation() {
        let manager = UserDependencyManager::new();
        let issues = manager.validate_solana_config().await
