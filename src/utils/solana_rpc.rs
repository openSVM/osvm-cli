...
use solana_sdk::{commitment_config::CommitmentConfig, clock::Slot};
...
pub async fn check_network_health(network: &str, custom_url: Option<&str>) -> Result<NetworkHealth> {
    ...
    match health_result {
        Ok(_) => {
            ...
            let (validator_count, voting_validators, stake_percent) = if let Some(accounts) = vote_accounts {
    ...
    loop {
        ...
        match get_network_stats(&client).await {
            Ok(stats) => {
                ...
                if slot_times.len() > 10 { // Keep last 10 samples
                    slot_times.pop_front();
                }
                
                let tps = if slot_diff > 0 { slot_diff as f64 * 400.0 } else { 0.0 }; // Rough approximation
...
async fn get_network_stats(client: &RpcClient) -> Result<NetworkStats> {
    ...
    let leader = client.get_slot_leaders(slot, 1).ok().and_then(|leaders| leaders.first().map(|l| l.to_string()));
...
pub async fn show_network_info(network: &str, custom_url: Option<&str>) -> Result<()> {
    ...
    println!("🔗 WebSocket: {}", SolanaNetworks::get_websocket_url(network));
    println!();
    
    // Get and display network health
    match check_network_health(network, custom_url).await {
        Ok(health) => {
            println!("🏥 Network Health: {}", if health.healthy { "✅ Healthy" } else { "❌ Unhealthy" });
...
mod tests {
    ...
    fn test_network_urls() {
        assert_eq!(SolanaNetworks::get_rpc_url("mainnet"), SolanaNetworks::MAINNET);
        assert_eq!(SolanaNetworks::get_rpc_url("devnet"), SolanaNetworks::DEVNET);
        assert_eq!(SolanaNetworks::get_rpc_url("testnet"), SolanaNetworks::TESTNET);
        assert_eq!(SolanaNetworks::get_rpc_url("invalid"), SolanaNetworks::MAINNET); // Default
    }
}