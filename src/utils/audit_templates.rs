...
use crate::utils::audit::{AuditReport, AuditFinding, AuditSeverity, AuditSummary, SystemInfo};
...
impl TemplateReportGenerator {
    ...
    pub fn generate_report(
        ...
        std::fs::write(output_path, rendered)
            .context("Failed to write rendered report")?;
        
        Ok(())
    }

    /// Generate Typst document from audit report
    pub fn generate_typst_document(
        &self,
        report: &AuditReport,
        output_path: &Path,
    ) -> Result<()> {
    ...
    fn create_template_context(&self, report: &AuditReport) -> Result<TeraContext> {
        ...
        context.insert("total_serious", &(report.summary.critical_findings + report.summary.high_findings));
        
        Ok(context)
    }

    /// Categorize findings by category
    fn categorize_findings<'a>(&self, findings: &'a [AuditFinding]) -> HashMap<String, Vec<&'a AuditFinding>> {
    ...
    fn group_by_severity<'a>(&self, findings: &'a [AuditFinding]) -> HashMap<String, Vec<&'a AuditFinding>> {
    ...
    fn calculate_statistics(&self, findings: &[AuditFinding]) -> serde_json::Value {
        ...
        let with_location = findings.iter().filter(|f| f.code_location.is_some()).count();
        
        let categories: std::collections::HashSet<_> = findings.iter().map(|f| &f.category).collect();
...
mod tests {
    ...
    fn test_fallback_analysis() {
        let fallback = EnhancedAIErrorHandler::generate_fallback_analysis(
            "Test Finding",
            "Security"
        );
