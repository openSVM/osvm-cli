...
use tokio::task;
use tokio::sync::mpsc;
use crate::utils::log_monitor::{LogMonitorConfig, monitor_logs_continuous};
...
fn find_agave_validator() -> Result<String> {
    ...
    let paths = vec![
        format!("{}/.local/share/solana/install/active_release/bin/agave-validator", home),
...
fn find_solana_keygen() -> Result<String> {
    ...
    // Check common installation paths
    let paths = vec![
        format!("{}/.local/share/solana/install/active_release/bin/solana-keygen", home),
...
async fn apply_system_tuning(param: &str, recommended_value: &str) -> Result<()> {
    println!("🔧 Applying system tuning: {} = {}", param, recommended_value);
...
pub async fn start_devnet_rpc_with_retry(config: DevnetRpcConfig) -> Result<DevnetRpcInfo> {
    ...
    loop {
        attempt += 1;
        println!("🚀 Starting devnet RPC (attempt {}/{})", attempt, max_attempts);
        ...
        match start_devnet_rpc_internal(config.clone(), attempt > 1).await {
            ...
            Err(e) => {
                ...
                if error_str.contains("OS network limit") || error_str.contains("too small") {
                    println!("⚠️  System tuning issue detected, will retry after applying fixes...");
...
async fn start_devnet_rpc_internal(config: DevnetRpcConfig, skip_pre_tuning: bool) -> Result<DevnetRpcInfo> {
    ...
    if !std::path::Path::new(&keypair_path).exists() {
        ...
        if !keygen_output.status.success() {
            anyhow::bail!("Failed to generate keypair: {}", String::from_utf8_lossy(&keygen_output.stderr));
        }
    ...
    cmd.arg("--entrypoint").arg("entrypoint.devnet.solana.com:8001");
    cmd.arg("--entrypoint").arg("entrypoint2.devnet.solana.com:8001");
    cmd.arg("--known-validator").arg("dv1ZAGvdsz5hHLwWXsVnM94hWf1pjbKVau1QVkaMJ92");
    cmd.arg("--known-validator").arg("dv2eQHeP4RFrJZ6UeiZWoc3XTtmtZCUKxxCApCDcRNV");
    cmd.arg("--known-validator").arg("dv4ACNkpYPcE3aKmYDqZm9G5EB3J4MRoeE7WNDRBVJB");
    cmd.arg("--known-validator").arg("dv3qDFk1DTF36Z62bNvrCXe9sKATA6xvVy6A798xxAS");
    cmd.arg("--expected-genesis-hash").arg("EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG");
    ...
    cmd.arg("--wal-recovery-mode").arg("skip_any_corrupted_record");
    ...
    if config.background {
        ...
        let restart_callback: Option<Box<dyn Fn() -> Result<()> + Send + Sync>> = Some(Box::new(|| {
