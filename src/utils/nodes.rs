...
use {
    crate::utils::{
        ssh_deploy::{AuthMethod, DeploymentConfig, NetworkType, ServerConfig, deploy_svm_node}, 
...
impl NodeDatabase {
    ...
    pub fn update_node(&mut self, id: &str, node: NodeInfo) -> Result<(), NodeError> {
        if !self.nodes.contains_key(id) {
            return Err(NodeError::NotFound(format!("Node with ID '{}' not found", id)));
        }
...
pub fn get_node_status(node_id: &str) -> Result<NodeStatus, Box<dyn Error>> {
    ...
    let status = rt.block_on(async {
        ...
        let status_output = client.execute_command(&format!("systemctl is-active {}", service_name))?;
...
pub fn display_node_list(nodes: &[NodeInfo], verbosity: u8) {
    ...
    // Print summary
    println!("Total Nodes: {} ({} running, {} stopped, {} error, {} unknown)",
        nodes.len(),
    ...
    println!("Node Types: {} validators, {} RPC nodes\n",
        validators.to_string().cyan(),
        rpc_nodes.to_string().cyan()
    );
    
    // Print table header
    println!("{:<16} {:<12} {:<10} {:<10} {:<16} {:<10}",
        "ID".blue().bold(), 
        "SVM".blue().bold(),
        "TYPE".blue().bold(),
        "NETWORK".blue().bold(),
    ...
    for node in nodes {
        ...
        if verbosity >= 2 && node.node_type == "rpc" && node.rpc_url.is_some() {
            println!("    {}: {}",
                "RPC URL".bright_black(), node.rpc_url.as_ref().unwrap().bright_cyan()
        );
        }
    } // End of for-loop
    ...
    println!("  - {}: Get detailed information about a specific node", "osvm node info <id>".cyan());
    println!("  - {}: Filter nodes by SVM type", "osvm node list --svm <name>".cyan());
    println!("  - {}: Filter nodes by network", "osvm node list --network <network>".cyan());
    println!("  - {}: Filter nodes by type", "osvm node list --type <validator|rpc>".cyan());
    println!("  - {}: Filter nodes by status", "osvm node list --status <running|stopped|error|unknown>".cyan());

    // Show additional tips for verbose mode
    if verbosity >= 2 {
        println!("\n{} Try increasing verbosity with -v, -vv, or -vvv for more details", "ADVANCED:".magenta().bold());
    }
}
...
pub fn display_node_info(node: &NodeInfo, verbosity: u8) {
    println!("\n{}", "OSVM - Node Management".cyan().bold());
    println!("{} {}", "Node Information:".green().bold(), node.id.cyan().bold());
    println!("{}", "====================".green());
    ...
    println!("{} running on {} network", node_type_display, node.network.to_string().yellow().bold());
    ...
    println!("  Created: {}", chrono::DateTime::from_timestamp(node.created_at as i64, 0)
    ...
    println!("  SSH Access: {}", format!("ssh user@{}", node.host).bright_white());
    ...
    if let Some(metrics) = &node.system_metrics {
        ...
        println!("  Network: {} kB/s in, {} kB/s out", metrics.network_in, metrics.network_out);
        println!("  Last Updated: {}", chrono::DateTime::from_timestamp(metrics.last_updated as i64, 0)
    ...
    if verbosity >= 3 {
        ...
        println!("  ID Format: {}-{}-{}-{}", node.svm_type, node.node_type, node.network, node.host);
    }
    ...
    match node.status {
        NodeStatus::Running => {
            println!("  {}: {}", "Stop Node".yellow(), format!("osvm node stop {}", node.id).cyan());
            println!("  {}: {}", "Restart Node".yellow(), format!("osvm node restart {}", node.id).cyan());
        },
        NodeStatus::Stopped => {
            println!("  {}: {}", "Start Node".green(), format!("osvm node start {}", node.id).cyan());
        },
        NodeStatus::Error => {
            println!("  {}: {}", "Start Node".green(), format!("osvm node start {}", node.id).cyan());
            println!("  {}: {}", "View Logs".yellow(), format!("osvm node logs {}", node.id).cyan());
        },
        NodeStatus::Unknown => {
            println!("  {}: {}", "Check Status".yellow(), format!("osvm node status {}", node.id).cyan());
        },
    }
    
    println!("  {}: {}", "View Logs".yellow(), format!("osvm node logs {}", node.id).cyan());
    println!("  {}: {}", "Remove Node".red(), format!("osvm node remove {}", node.id).cyan());
}
...
pub fn display_node_status(node_id: &str, status: &NodeStatus, verbosity: u8) {
    ...
    match status {
        NodeStatus::Running => {
            println!("  {}: {}", "Stop Node".yellow(), format!("osvm node stop {}", node_id).cyan());
            println!("  {}: {}", "Restart Node".yellow(), format!("osvm node restart {}", node_id).cyan());
        },
        NodeStatus::Stopped => {
            println!("  {}: {}", "Start Node".green(), format!("osvm node start {}", node_id).cyan());
        },
        NodeStatus::Error => {
            println!("  {}: {}", "View Logs".red(), format!("osvm node logs {}", node_id).cyan());
            println!("  {}: {}", "Restart Node".yellow(), format!("osvm node restart {}", node_id).cyan());
        },
        NodeStatus::Unknown => {}
    }
    ...
    if verbosity >= 2 {
        ...
        println!("{}", "--------------------".blue());
        println!("  Status check performed at: {}", chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string());
    }
}
...
pub fn run_dashboard(
    ...
    // Get all nodes
    let nodes = list_all_nodes(client, "all", None, "all", "all", commitment_config, verbosity)?;
