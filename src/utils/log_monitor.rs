...
use crate::{osvm_info, osvm_error, osvm_warn, osvm_debug};
...
async fn install_ngrok() -> Result<()> {
    ...
    if output.status.success() {
        ...
        if alt_output.status.success() {
            ...
            anyhow::bail!("Failed to install ngrok: {}", String::from_utf8_lossy(&output.stderr))
        }
    }
}
...
async fn setup_ngrok_tunnels(ports: &[u16]) -> Result<HashMap<u16, String>> {
    osvm_info!(LogCategory::Ngrok, "setup_ngrok_tunnels", &format!("Setting up ngrok tunnels for ports: {:?}", ports));
    ...
    if !is_ngrok_installed() {
        osvm_warn!(LogCategory::Ngrok, "setup_ngrok_tunnels", "ngrok not found, installing automatically");
    ...
    for &port in ports {
                            ...
                            if let Some(addr) = config["addr"].as_str() {
                                if addr.ends_with(&format!(":{}", port)) {
                                    if let Some(public_url) = tunnel["public_url"].as_str() {
                                        ...
                                        osvm_info!(LogCategory::Ngrok, "setup_ngrok_tunnels", &format!("Successfully created tunnel for port {}: {}", port, public_url));
                                        println!("✅ ngrok tunnel for port {}: {}", port, public_url);
                                        break;
                                    }
                                }
                            }
        ...
        if !tunnel_urls.contains_key(&port) {
            println!("⚠️  Created ngrok tunnel for port {} (URL not retrieved)", port);
        }
    }
...
async fn apply_fix(issue: &DetectedIssue, config: &LogMonitorConfig) -> Result<FixResult> {
    osvm_info!(LogCategory::AutoRepair, "apply_fix", &format!("Attempting to fix issue: {:?}", issue.issue_type));
    
    if !config.auto_fix_enabled {
        osvm_warn!(LogCategory::AutoRepair, "apply_fix", "Auto-fix is disabled, skipping repair");
        return Ok(FixResult {
            success: false,
    ...
    match &issue.issue_type {
        IssueType::SystemTuning(param) => {
            ...
            let recommended = issue.context.get("recommended")
            ...
            if output.status.success() {
                ...
                Ok(FixResult {
                    success: false,
                    message: format!("Failed to set {}: {}", param, String::from_utf8_lossy(&output.stderr)),
                    requires_restart: false,
                })
            }
        }
        ...
        IssueType::TooManyOpenFiles => {
            ...
            let _ = Command::new("sh")
                .arg("-c")
                .arg(&format!("echo '{}' | sudo tee /etc/security/limits.d/90-nofile.conf", limits_conf))
                .output();
        ...
        IssueType::NetworkConnectivity => {
            ...
            for port in &ports {
                ...
                if let Ok(result) = output {
                    if result.status.success() {
                        ...
                        println!("⚠️  Failed to open port {}/tcp: {}", port, String::from_utf8_lossy(&result.stderr));
                    }
                }
            }
            ...
            Ok(FixResult {
                ...
                message: format!("Opened {}/{} firewall ports for Solana validator", success_count, ports.len()),
                requires_restart: true,
            })
        }
        
        IssueType::ExternalReachability => {
            osvm_info!(LogCategory::Ngrok, "apply_fix", "External reachability issue detected - setting up ngrok tunnels");
            println!("🌐 Fixing external reachability issue - setting up ngrok tunnels...");
            
            // Get the port from context or use default Solana ports
            let failed_port = issue.context.get("port")
                .and_then(|p| p.parse::<u16>().ok())
                .unwrap_or(8899);
                
            osvm_debug!(LogCategory::Ngrok, "apply_fix", &format!("Failed port: {}, setting up tunnels for Solana ports", failed_port));
            ...
            if let Some(captures) = pattern.regex.captures(&line) {
                ...
                let issue = DetectedIssue {
                    issue_type: match &pattern.issue_type {
                        IssueType::SystemTuning(_) => {
                            IssueType::SystemTuning(context.get("parameter").cloned().unwrap_or_default())
                        }
                        IssueType::PortConflict(_) => {
                ...
                println!("🔍 Detected issue: {:?} (Severity: {:?})", issue.issue_type, issue.severity);
                ...
                match apply_fix(&issue, &config).await {
                    Ok(fix_result) => {
                        if fix_result.success {
                            ...
                            *state_guard.issues_fixed.entry(issue.issue_type.clone()).or_insert(0) += 1;
                            ...
                            if fix_result.requires_restart && config.restart_on_critical {
                                ...
                                if should_restart && state_guard.restart_count < config.max_restart_attempts {
...
pub async fn monitor_log_file(
    ...
    osvm_info!(LogCategory::AutoRepair, "monitor_log_file", &format!("Starting real-time log monitoring for: {}", log_path));
            ...
            Ok(_) => {
                ...
                for pattern in patterns.iter() {
                    if let Some(captures) = pattern.regex.captures(&line) {
                        ...
                        osvm_info!(LogCategory::AutoRepair, "monitor_log_file", &format!("ISSUE DETECTED: {:?} (Severity: {:?}) - Message: {}", issue.issue_type, issue.severity, issue.message));
                        println!("🔍 Detected issue: {:?} (Severity: {:?})", issue.issue_type, issue.severity);
                        ...
                        match apply_fix(&issue, &config).await {
                            Ok(fix_result) => {
                                if fix_result.success {
                                    osvm_info!(LogCategory::AutoRepair, "monitor_log_file", &format!("FIX SUCCESSFUL: {}", fix_result.message));
                                    println!("✅ Fix applied: {}", fix_result.message);
