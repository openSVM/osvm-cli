...
use crate::{osvm_info, osvm_error};
...
pub async fn start_mainnet_rpc(config: MainnetRpcConfig) -> Result<MainnetRpcInfo> {
    osvm_info!(LogCategory::Rpc, "start_mainnet_rpc", "Starting mainnet-beta RPC node");
    ...
    cmd.arg("--identity").arg("/home/larp/.config/solana/id.json")
       .arg("--ledger").arg(&config.ledger_path)
       .arg("--rpc-port").arg(config.rpc_port.to_string())
       .arg("--gossip-port").arg(config.gossip_port.to_string())
       .arg("--dynamic-port-range").arg("8002-8020")
       .arg("--log").arg("-");  // Log to stdout
    ...
    cmd.arg("--known-validator").arg("7Np41oeYqPefeNQEHSv1UDhYrehxin3NStELsSKCT4K2")  // Figment
       .arg("--known-validator").arg("GdnSyH3YtwcxFvQrVVJMm1JhTS4QVX7MFsX56uJLUfiZ")  // Figment
       .arg("--known-validator").arg("DE1bawNcRJB9rVm3buyMVfr8mBEoyyu73NBovf2oXJsJ")  // Bison Trails
       .arg("--known-validator").arg("CakcnaRDHka2gXyfbEd2d3xsvkJkqsLw2akB3zsN1D2S")  // Canonical
       .arg("--expected-genesis-hash").arg("5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d");
    ...
    cmd.arg("--no-voting")  // Don't vote, we're just an RPC node
       ...
       .arg("--rpc-bind-address").arg("0.0.0.0")  // Bind to all interfaces
       .arg("--private-rpc")  // Don't register with public RPC list
       .arg("--full-rpc-api")  // Enable all RPC methods
       .arg("--account-index").arg("program-id")  // Index by program ID
       .arg("--account-index-exclude-key").arg("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")  // Exclude token accounts
       .arg("--account-index-exclude-key").arg("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb");  // Exclude token-2022 accounts
    
    // Performance settings
    cmd.arg("--limit-ledger-size").arg("500000000")  // Limit to ~250GB to start
       .arg("--accounts-db-cache-limit-mb").arg("10000")  // 10GB accounts cache
       .arg("--max-genesis-archive-unpacked-size").arg("10737418240")  // 10GB max genesis
    ...
    if config.background {
        ...
        // Create log file
        let log_file_path = format!("agave-validator-mainnet-{}.log", chrono::Utc::now().timestamp());
        ...
        tokio::spawn(async move {
            if let Err(e) = monitor_log_file(&log_path_clone, log_monitor_config, None).await {
                osvm_error!(LogCategory::Rpc, "start_mainnet_rpc", &format!("Log monitor error: {}", e));
            }
        });
...
pub async fn stop_mainnet_rpc() -> Result<()> {
    osvm_info!(LogCategory::Rpc, "stop_mainnet_rpc", "Stopping mainnet RPC node");
