...
pub fn generate_monitoring_dashboard(nodes: &[NodeInfo], verbosity: u8) -> String {
    ...
    html.push_str(&format!("    <div class=\"last-update\">Last updated: {}</div>\n", current_time));
    
    // Add verbosity indicator if debug level
    if verbosity >= 3 {
        html.push_str(&format!("    <div style=\"margin-left: 20px; color: #888;\">\n"));
        html.push_str(&format!("        <small>Debug mode: Verbosity level {}</small>\n", verbosity));
    ...
    html.push_str(&format!("                <div class=\"stat-label\">Total Nodes</div>\n"));
    html.push_str(&format!("            </div>\n"));
    
    html.push_str(&format!("            <div class=\"stat-box\" style=\"background-color: #e6f4ea;\">\n"));
    html.push_str(&format!("                <div class=\"stat-value\" style=\"color: #34a853;\">{}</div>\n", running));
    html.push_str(&format!("                <div class=\"stat-label\">Running</div>\n"));
    html.push_str(&format!("            </div>\n"));
    
    html.push_str(&format!("            <div class=\"stat-box\" style=\"background-color: #fef7e0;\">\n"));
    html.push_str(&format!("                <div class=\"stat-value\" style=\"color: #fbbc05;\">{}</div>\n", stopped));
    html.push_str(&format!("                <div class=\"stat-label\">Stopped</div>\n"));
    html.push_str(&format!("            </div>\n"));
    
    html.push_str(&format!("            <div class=\"stat-box\" style=\"background-color: #fce8e6;\">\n"));
    html.push_str(&format!("                <div class=\"stat-value\" style=\"color: #ea4335;\">{}</div>\n", error));
    html.push_str(&format!("                <div class=\"stat-label\">Error</div>\n"));
    html.push_str(&format!("            </div>\n"));
    
    html.push_str(&format!("            <div class=\"stat-box\" style=\"background-color: #f1f3f4;\">\n"));
    html.push_str(&format!("                <div class=\"stat-value\" style=\"color: #9aa0a6;\">{}</div>\n", unknown));
    html.push_str(&format!("                <div class=\"stat-label\">Unknown</div>\n"));
    html.push_str(&format!("            </div>\n"));
    html.push_str("        </div>\n");
    html.push_str("    </div>\n");
    ...
    if verbosity >= 3 {
        ...
        html.push_str(&format!("Dashboard generated with verbosity level: {}\nTotal nodes: {}\nTimestamp: {}\n", verbosity, nodes.len(), current_time));
        html.push_str("    </pre>\n");
        html.push_str("</div>\n");
    }
