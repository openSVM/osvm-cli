...
use crate::utils::audit::{CodeSnippet, AuditFinding};
...
impl CodeExtractor {
    ...
    pub fn extract_snippet_around_line(
        ...
        if target_line == 0 || target_line > total_lines {
            anyhow::bail!("Invalid line number: {} (file has {} lines)", target_line, total_lines);
        }
    ...
    pub fn extract_snippet_by_pattern(
        ...
        for (line_num, line) in lines.iter().enumerate() {
            if line.contains(pattern) {
                ...
                if let Ok(snippet) = Self::extract_snippet_around_line(file_path, target_line, context_lines) {
        ...
        match finding.category.as_str() {
            ...
            _ => {
                ...
                if content.contains("unwrap()") {
                    fixed_content = fixed_content.replace("unwrap()", "map_err(|_| ErrorCode::OperationFailed)?");
                }
            }
        }
...
mod tests {
    ...
    fn test_extract_snippet_around_line() {
        ...
        let snippet = CodeExtractor::extract_snippet_around_line(
    ...
    fn test_extract_snippet_by_pattern() {
        ...
        let snippets = CodeExtractor::extract_snippet_by_pattern(
            temp_file.path().to_str().unwrap(),
            "fn ",
            1
        ).unwrap();
    ...
    fn test_generate_suggested_fix() {
        let problematic_code = CodeSnippet {
            ...
            start_line: 1,
            end_line: 2,
            content: "let user_account = &ctx.accounts.user_account;\nuser_account.balance += amount;".to_string(),
            context: None,
        };
        ...
        let fixed_code = CodeExtractor::generate_suggested_fix(&problematic_code, &finding).unwrap();
