...
pub fn render_markdown(text: &str) -> String {
    ...
    if let Ok(bold_re) = Regex::new(r"\*\*([^*]+)\*\*") {
        rendered = bold_re
            .replace_all(&rendered, "\x1b[1m$1\x1b[0m")
            .to_string();
    }
    ...
    if let Ok(italic_re) = Regex::new(r"\*([^*]+)\*") {
        rendered = italic_re
            .replace_all(&rendered, "\x1b[3m$1\x1b[0m")
            .to_string();
    }

    // Convert code blocks
    if let Ok(code_block_re) = Regex::new(r"```([^`]*)```") {
        rendered = code_block_re
            .replace_all(&rendered, "\x1b[48;5;236m$1\x1b[0m")
            .to_string();
    }

    // Convert inline code
    if let Ok(inline_code_re) = Regex::new(r"`([^`]+)`") {
        rendered = inline_code_re
            .replace_all(&rendered, "\x1b[7m$1\x1b[0m")
            .to_string();
    }
