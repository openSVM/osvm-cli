...
mod tests {
    ...
    async fn setup_docker_container() -> Result<String, Box<dyn std::error::Error>> {
        ...
        if !output.status.success() {
            return Err(format!("Failed to start Docker container: {}", String::from_utf8_lossy(&output.stderr)).into());
        }
    ...
    async fn cleanup_docker_container(container_id: &str) -> Result<(), Box<dyn std::error::Error>> {
        let status = Command::new("docker")
