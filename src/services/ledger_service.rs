...
use rocksdb::{ColumnFamily, IteratorMode, Options as RocksDBOptions, DB};
...
use crate::services::transaction_decoders::{
    format_decoded_instruction, DecodedInstruction, InstructionDecoderRegistry,
};
...
impl LedgerService {
    pub fn new(ledger_path: PathBuf) -> Result<Self> {
        ...
        let (db, cf_handles) = if ledger_path.join("rocksdb").exists() {
            match Self::open_rocksdb(&ledger_path) {
                ...
                Err(e) => {
                    warn!(
                        "Failed to open RocksDB: {}. Transaction history will not be available.",
                        e
                    );
                    (None, HashMap::new())
                }
            ...
            debug!(
                "RocksDB not found at {:?}. Transaction history not available.",
                ledger_path
            );
            (None, HashMap::new())
        };
    ...
    pub async fn get_transaction(&self, signature: &str) -> Result<TransactionInfo> {
        ...
        match db.get_cf(cf_handle, &sig_bytes) {
            Ok(Some(data)) => {
                ...
                let (fee, success, logs, accounts) = rocksdb_parser::parse_transaction_metadata(
                    &data, offset,
                )
    ...
    pub async fn stream_transactions_for_address(
        ...
        if !self.has_transaction_history() {
            return Err(anyhow!("Ledger does not contain transaction history"));
        }
        ...
        info!(
            "Streaming up to {} transactions for address: {}",
            limit, address
        );

        // Parse address to pubkey
        let pubkey = Pubkey::from_str(address).map_err(|e| anyhow!("Invalid address: {}", e))?;
        ...
        info!(
            "Found {} transactions for address {}",
            transactions.len(),
            address
        );
        Ok(transactions)
    ...
    pub async fn stream_transactions_in_range(
        ...
        info!(
            "Streaming up to {} transactions from slot {} to {}",
            limit, start_slot, end_slot
        );
        ...
        info!(
            "Found {} transactions in slot range {}-{}",
            transactions.len(),
            start_slot,
            end_slot
        );
    ...
    async fn detect_new_slot(
        ...
        if let Ok(metadata) = tokio::fs::metadata(ledger_path.join("rocksdb")).await {
            ...
            let estimated_slot = (current_size / 1000000);

            if last_slot.is_none_or(|last| estimated_slot > last) {
