...
impl VsockConnectionPool {
    ...
    fn clear(&mut self) {
        debug!(
            "Clearing connection pool (had {} connections)",
            self.connections.len()
        );
...
impl OsvmMicroVmHandle {
    ...
    pub fn shutdown(mut self) -> Result<()> {
        ...
        if self.is_running() {
            ...
            self.child
                .kill()
                .context("Failed to kill microVM process")?;
        }

        self.child
            .wait()
            .context("Failed to wait for microVM process")?;
    ...
    fn send_shutdown_signal(&self) -> Result<()> {
        ...
        let url = "http://localhost/actions".to_string();
...
impl MicroVmLauncher {
    ...
    pub fn launch_osvm_runtime(&self, config: OsvmMicroVmConfig) -> Result<OsvmMicroVmHandle> {
        ...
        let api_socket = self
            .runtime_dir
            .join(format!("osvm-runtime-{}.sock", std::process::id()));
        ...
        let config_file = self
            .runtime_dir
            .join(format!("osvm-config-{}.json", std::process::id()));
        let config_json = serde_json::to_string_pretty(&firecracker_config)?;
        std::fs::write(&config_file, config_json).context("Failed to write Firecracker config")?;
...
impl McpServerMicroVmHandle {
    ...
    pub async fn health_check_active(&self) -> Result<()> {
        ...
        // Read response
        let mut len_bytes = [0u8; 4];
        timeout(Duration::from_secs(5), reader.read_exact(&mut len_bytes))
            .await
    ...
    pub fn shutdown(mut self) -> Result<()> {
        info!(
            "Shutting down MCP server microVM: {}",
            self.config.server_id
        );
        ...
        if self.is_running() {
            ...
            self.child
                .kill()
                .context("Failed to kill MCP server microVM process")?;
        }
        ...
        info!(
            "MCP server microVM shut down successfully: {}",
            self.config.server_id
        );
        Ok(())
    }
...
impl MicroVmLauncher {
    ...
    pub fn launch_mcp_server(
        ...
        info!(
            "Launching MCP server '{}' in dedicated microVM",
            config.server_id
        );
        debug!("Configuration: {:?}", config);
        ...
        let api_socket = self
            .runtime_dir
            .join(format!("mcp-{}-api.sock", config.server_id));
        let vsock_socket = self
            .runtime_dir
            .join(format!("mcp-{}-vsock.sock", config.server_id));

        // 4. Build Firecracker configuration
        let firecracker_config =
            self.build_mcp_server_firecracker_config(&config, &api_socket, &vsock_socket)?;

        // 5. Write config file
        let config_file = self
            .runtime_dir
            .join(format!("mcp-{}-config.json", config.server_id));
        let config_json = serde_json::to_string_pretty(&firecracker_config)?;
        ...
        info!(
            "Starting Firecracker microVM for MCP server '{}'...",
            config.server_id
        );
        let child = cmd
            .spawn()
...
mod tests {
    ...
    fn test_vsock_cid_allocation_uniqueness() {
        ...
        assert!(
            cids.len() >= 35,
            "Expected good CID distribution, got {} unique CIDs",
            cids.len()
        );
    }
    ...
    fn test_build_mcp_server_firecracker_config() {
        ...
        let firecracker_config =
            launcher.build_mcp_server_firecracker_config(&config, &api_socket, &vsock_socket);
        ...
        assert_eq!(
            vsock_config.get("guest_cid").unwrap().as_u64().unwrap(),
            100
        );
        ...
        assert_eq!(
            machine_config.get("vcpu_count").unwrap().as_u64().unwrap(),
            1
        );
        assert_eq!(
            machine_config
    ...
    fn test_mcp_server_command_escaping() {
        ...
        let fc_config = launcher
            .build_mcp_server_firecracker_config(&config, &api_socket, &vsock_socket)
            .unwrap();

        let boot_args = fc_config
            .get("boot-source")
