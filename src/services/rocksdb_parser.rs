...
pub fn parse_transaction_metadata(
    ...
    debug!("Extracted {} logs, {} accounts from metadata", logs.len(), accounts.len());
    ...
    while pos + 8 <= data.len() {
        ...
        if vec_len > 0 && vec_len < 1000 {
            ...
            for _ in 0..vec_len {
                ...
                let str_len = u64::from_le_bytes(data[pos..pos+8].try_into().unwrap_or([0; 8])) as usize;
                pos += 8;
                ...
                if str_len > 0 && str_len < 10000 && pos + str_len <= data.len() {
                    if let Ok(log_msg) = String::from_utf8(data[pos..pos+str_len].to_vec()) {
                        ...
                        if log_msg.contains("Program") || log_msg.contains("invoke") || log_msg.contains("success") {
                            logs.push(log_msg);
                        }
                    }
                    pos += str_len;
