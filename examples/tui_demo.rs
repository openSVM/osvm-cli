...
fn main() -> anyhow::Result<()> {
    ...
    // Add some mock agent output
    app.add_agent_output("🚀 Starting Intelligent Wallet Research...".to_string());
    app.add_agent_output("✅ MCP tools initialized (84 tools loaded)".to_string());
    app.add_agent_output("🔍 DEBUG: Iteration #1 - get_account_stats".to_string());
    app.add_agent_output("🔍 DEBUG: Iteration #2 - get_account_transfers".to_string());
    app.add_agent_output("✅ Transfer data fetched successfully".to_string());

    // Add some mock logs
    app.add_log("MCP server 'osvm-mcp' connected".to_string());
    app.add_log("Brotli compression enabled".to_string());
    app.add_log("Executing OVSM script for get_account_transfers".to_string());
