...
use ovsm::{Evaluator, Parser, Scanner, Value, Error};
...
fn test_try_catch_actually_catches_errors() {
    ...
    assert_eq!(result, Value::Int(0), "TRY-CATCH must actually catch errors");
}
...
fn test_parallel_errors_loudly() {
    ...
    assert!(err_msg.contains("NotImplemented") || err_msg.contains("PARALLEL"),
        "Error must mention PARALLEL or NotImplemented, got: {}", err_msg);
}
...
fn test_decision_errors_loudly() {
    ...
    let err_msg = format!("{:?}", result.unwrap_err());
    assert!(err_msg.contains("NotImplemented") || err_msg.contains("DECISION"),
        "Error must mention DECISION or NotImplemented, got: {}", err_msg);
}
...
fn test_wait_strategy_errors_loudly() {
    ...
    let err_msg = format!("{:?}", result.unwrap_err());
    assert!(err_msg.contains("NotImplemented") || err_msg.contains("WAIT") || err_msg.contains("WaitStrategy"),
        "Error must mention WAIT/WaitStrategy or NotImplemented, got: {}", err_msg);
}
...
fn test_multiple_guards_stop_at_first_failure() {
    ...
    assert_eq!(result, Value::String("guard2".to_string()),
...
fn test_guard_with_true_condition() {
    ...
    assert_eq!(result, Value::String("passed".to_string()),
...
fn test_try_catch_with_tool_error() {
    ...
    assert_eq!(result, Value::Int(0),
        "TRY-CATCH must catch tool errors");
}
